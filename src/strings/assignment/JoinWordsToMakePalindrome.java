 ...
 * palindrome problem we did in the class] Given a list of words, is there any
 * pair of words, that can be joined (in any order) to form a palindrome?
 * Example 1:- Consider a list {bat, tab, cat}. Then bat and tab can be joined
 * together to form a palindrome. Example 2 :- {ab, deedba} can be joined to
 * form a palindrome. Example 3 :- {'ant', 'cat', 'dog'}: No two words here can
 * be joined to form a palindrome. There can be multiple pairs, just return the
 * first one if found. About expected solutions: Given n = number of words and k
 * is length of the longest word, O(N.K^2) solution for this problem is
 * relatively straightforward to come by, using Maps or Tries. That may suffice
 * for most interviews. e.g.
