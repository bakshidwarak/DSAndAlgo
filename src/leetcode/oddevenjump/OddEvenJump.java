 ...
 * Favorite
 * 
 * Share You are given an integer array A. From some starting index, you can
 * make a series of jumps. The (1st, 3rd, 5th, ...) jumps in the series are
 * called odd numbered jumps, and the (2nd, 4th, 6th, ...) jumps in the series
 ...
 * Example 1:
 * 
 * Input: [10,13,12,14,15] Output: 2 Explanation: From starting index i = 0, we
 * can jump to i = 2 (since A[2] is the smallest among A[1], A[2], A[3], A[4]
 * that is greater or equal to A[0]), then we can't jump any more. From starting
 * index i = 1 and i = 2, we can jump to i = 3, then we can't jump any more.
 * From starting index i = 3, we can jump to i = 4, so we've reached the end.
 * From starting index i = 4, we've reached the end already. In total, there are
 * 2 different starting indexes (i = 3, i = 4) where we can reach the end with
 * some number of jumps. Example 2:
 * 
 * Input: [2,3,1,1,4] Output: 3 Explanation: From starting index i = 0, we make
 * jumps to i = 1, i = 2, i = 3:
 * 
 ...
 * 
 * In a similar manner, we can deduce that: From starting index i = 1, we jump
 * to i = 4, so we reach the end. From starting index i = 2, we jump to i = 3,
 * and then we can't jump anymore. From starting index i = 3, we jump to i = 4,
 * so we reach the end. From starting index i = 4, we are already at the end. In
 * total, there are 3 different starting indexes (i = 1, i = 3, i = 4) where we
 * can reach the end with some number of jumps. Example 3:
 * 
 * Input: [5,1,3,4,2] Output: 3 Explanation: We can reach the end from starting
 * indexes 1, 2, and 4.
 * 
 * 
 * Note:
